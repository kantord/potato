...
async fn render_nonexistent_script_returns_error() {
    ...
    let response = app
        .oneshot(
            Request::post("/__render/nonexistent.sh")
                .header("Content-Type", "application/json")
                .body(Body::from("{}"))
                .unwrap(),
        )
...
async fn render_traversal_cannot_execute_arbitrary_binaries() {
    ...
    let response = app
        .oneshot(
            Request::post("/__render/..%2f..%2fbin%2fdate")
                .header("Content-Type", "application/json")
                .body(Body::from("{}"))
                .unwrap(),
        )
